...
const AdSenseAd = () => {
  return (
    <div className="bg-red my-8 flex justify-center">
      <script
        async
...
function injectAdsIntoContent(content: string): string {
  ...
  if (wordCount > 2000) {
    // For very long articles, add ads after every 500 words or after h2 headings
    return content.replace(/^(#{1,2} .+\n\n)/gm, `${adPlaceholder}\n\n`)
  } else if (wordCount > 500) {
    // For medium articles, add ads after every h2 (keeping every 2nd)
    let headingCount = 0
    return content.replace(/^(## .+\n\n)/gm, () => {
      headingCount++
      return headingCount % 2 === 0 ? `${adPlaceholder}\n\n` : `$1`
    })
  }
