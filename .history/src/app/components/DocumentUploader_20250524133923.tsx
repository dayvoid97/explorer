...
export default function DocumentUploader({
  ...
  const [previewType, setPreviewType] = useState<'text' | 'pdf' | 'image' | null>(null)
  ...
  useEffect(() => {
    return () => {
      if (previewContent && (previewType === 'pdf' || previewType === 'image')) {
  ...
  const generatePreview = async (file: File) => {
    ...
    try {
      if (fileType === 'text/plain' || fileType === 'text/csv') {
        ...
        // PDF files - create object URL for embed
        ...
        // For other file types (Word, Excel, PowerPoint), show file info
        setPreviewContent(
          `This file type (${fileType}) cannot be previewed directly. You can download it to view the contents.`
        )
        setPreviewType('text')
      }
    } catch (error) {
      console.error('Error generating preview:', error)
      setPreviewContent('Unable to generate preview for this file.')
      setPreviewType('text')
    }
  }
  ...
  const removeFile = () => {
    ...
    setError(null)
    closePreview() // Clean up preview when removing file
    onFileRemove?.()
