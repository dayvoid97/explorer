...
const SpinWheelBuilder: React.FC = () => {
  ...
  const [wheelDescription, setWheelDescription] = useState<string>('Enter a description')
  ...
  useEffect(() => {
    ...
    const handleTransitionEnd = () => {
      // Compute winner after spin ends
      const totalRotation = rotation % 360
      const sectionAngle = 360 / wheelSections.length
      // Pointer is at 90° offset (top)
      const adjustedDegree = (totalRotation + 90) % 360
      const winnerIndex = Math.floor((360 - adjustedDegree) / sectionAngle) % wheelSections.length
  ...
  const spinWheel = (): void => {
    ...
    const randomSpins = Math.floor(Math.random() * 5) + 100 // 5–9 full spins
  ...
  const createWheelSVG = (): JSX.Element[] => {
    ...
    return wheelSections.map((section: WheelSection, index: number) => {
      ...
      const textAngle = startAngle + (sectionAngle * Math.PI) / 360
      ...
      return (
        <g key={index}>
          <path
            d={pathData}
            fill={`hsl(${(index * 360) / wheelSections.length}, 70%, 65%)`}
            stroke="#fff"
            strokeWidth="2"
          />
          <text
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white min-h-screen">
      <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">SPIN THE WHEEL BUILDER</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          <div className="w-full max-w-sm">
            ...
            <label
              htmlFor="logo-upload"
              className="w-full h-32 bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50"
            >
              {logoImage ? (
                <img
                  src={logoImage}
                ...
                <>
                  ...
                  <p className="text-gray-500 text-sm font-medium">INSERT WHEEL LOGO HERE</p>
                </>
              )}
            </label>
          </div>
          ...
          <div className="w-full max-w-sm">
            <textarea
              ...
              className="w-full bg-black text-white p-3 rounded-lg resize-none border-none outline-none text-center"
          ...
          <div className="relative">
            <svg width="400" height="400" className="drop-shadow-md">
              ...
              {/* Center circle */}
              <circle cx="200" cy="200" r="15" fill="#374151" stroke="#fff" strokeWidth="2" />
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border">
            <h3 className="font-bold text-lg mb-3 text-gray-800">Number of Prizes</h3>
            <select
            ...
            <div className="space-y-3">
              {wheelSections.map((section, index) => (
                <div key={index} className="flex items-center gap-3">
                  <span className="font-medium w-8 text-sm">{index + 1}.</span>
                  <input
                    type="text"
                    value={section.text}
                    ...
                    className="flex-1 px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500"
