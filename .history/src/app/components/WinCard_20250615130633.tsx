...
export default function WinCard({ win }: WinProps) {
  ...
  return (
    <div
      ...
      // Removed max-h and overflow from the main card if it's always meant to be a flexible preview
      // If you want fixed height cards in the grid, you'll need to re-add these or similar on the parent grid items.
    >
      {/* Header */}
        ...
        <div className="flex-grow min-w-0">
          ...
          <div className={expanded ? '' : 'line-clamp-5'}>
            {win.paragraphs.map((p, i) => (
              <p
                key={i}
                className="text-sm text-gray-800 dark:text-gray-300 whitespace-pre-line leading-relaxed"
              >
                {p}
              </p>
            ))}
