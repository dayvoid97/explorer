...
import { Plus, Image, Video, FileText, BarChart2, Upload, Volume2 } from 'lucide-react'
...
export default function MediaUploader({ cardId }: { cardId: string }) {
  ...
  const mediaOptions = [
    ...
    { label: 'Audio', icon: <Volume2 size={16} />, value: 'audio' }, // ✅ NEW
  ...
  const renderContentInput = () => {
    switch (type) {
      ...
      case 'audio':
        return (
          <div className="space-y-2">
            <label className="text-foreground text-sm font-medium">Audio File</label>
            <MultiMediaUploader
              acceptedTypes="audio"
              maxSizeInMB={100}
              onFileSelect={(selectedFile, metadata) => {
