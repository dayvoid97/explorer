  ...
  const interact = async (type: 'like' | 'save' | 'repost' | 'share') => {
    ...
    try {
      ...
      const res = await Promise.race([
        authFetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/gurkha/chronology/${id}/${type}`, {
          method: 'POST',
        }),
        timeoutPromise,
      ])
      ...
      if (!(res instanceof Response)) throw new Error('timeout')
      ...
      // 🧠 Fallback to sample data if timeout or network issue
      if (err.message.includes('timeout') || err.message.includes('Failed to fetch')) {
        console.warn('⚠️ API took too long — using sampleChronologyCards fallback')
        const fallbackChrono = sampleChronologyCards.find((c) => c.id === id)
        if (fallbackChrono) {
          if (type === 'like') {
            setIsLiked((prev) => !prev)
            showNotification('❤️ (Offline) Like toggled', 'success')
          } else if (type === 'save') {
            setIsSaved((prev) => !prev)
            showNotification('📚 (Offline) Save toggled', 'success')
          } else if (type === 'repost') {
            setIsReposted((prev) => !prev)
            showNotification('🔄 (Offline) Repost toggled', 'success')
          }
        }
        return
      }
      ...
      if (err.message.includes('Authentication required') || err.message.includes('log in again')) {
        if (onUnauthenticatedInteraction) onUnauthenticatedInteraction()
      } else {
