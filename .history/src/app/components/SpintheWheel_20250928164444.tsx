        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-300 text-lg">
              Create, customize, and share your spinning wheel of fortune
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            <div className="xl:col-span-2 flex flex-col items-center space-y-8">
              ...
              {selectedWinner && (
                <div className="text-center p-2 bg-gradient-to-r from-yellow-400/20 to-orange-500/20 backdrop-blur-lg rounded-2xl shadow-xl border border-yellow-400/30">
                  <p className="text-yellow-300 text-xl mb-3">🎉 Congratulations! 🎉</p>
                  <p className="text-4xl font-bold text-white mb-2">{selectedWinner.name}</p>
                  <div
                    className="w-6 h-6 rounded-full mx-auto border-2 border-white/50"
                    style={{ backgroundColor: selectedWinner.color }}
                  />
                </div>
              )}
                    ...
                    {participants.map((participant, index) => (
                      ...
                      >
                        <input
                          ...
                          className="w-10 h-10 rounded-lg border-2 border-white/30 cursor-pointer"
                        />
                        <input
                          type="text"
                          value={participant.name}
                          onChange={(e) => updateParticipant(index, 'name', e.target.value)}
                          className="flex-1 px-4 py-3 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-gray-400"
                          placeholder="Enter name"
                        />
                        <button
                          onClick={() => removeParticipant(index)}
                          disabled={participants.length <= 2}
                          className="flex items-center justify-center w-10 h-10 bg-red-500/80 hover:bg-red-600 disabled:bg-gray-600 text-white rounded-lg transition-all"
                        >
                          <Minus size={16} />
                        </button>
                      </div>
