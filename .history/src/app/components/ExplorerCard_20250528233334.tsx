...
export default function ExplorerCard({ item, onRemove, onSave }: ExplorerCardProps): JSX.Element {
  ...
  return (
    <div className="relative p-6 border border-gray-200 rounded-xl shadow-md bg-white dark:bg-gray-900">
      ...
      <div className="relative top-3 right-3 flex gap-20">
        <button
          title="Explore"
          onClick={navigateToCompanyPage}
          className="px-5 py-2 rounded-lg bg-white text-black font-semibold shadow-md hover:bg-black hover:text-white transition-colors duration-200"
        >
          Explore {item.ticker} Company Card
        </button>
      </div>
    </div>
  )
