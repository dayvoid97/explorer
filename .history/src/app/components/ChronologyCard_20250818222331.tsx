...
export default function ChronologyCard(props: ChronologyCardHydrated) {
  ...
  // Update state when props change (for SSR/hydration)
  React.useEffect(() => {
    console.log('ChronologyCard props update:', {
      id,
      name,
      savedByUser,
      likedByUser,
      repostedByUser,
      hasProps: {
        savedByUser: savedByUser !== undefined,
        likedByUser: likedByUser !== undefined,
        repostedByUser: repostedByUser !== undefined,
      },
      propValues: {
    ...
    setIsReposted(!!repostedByUser)
  }, [id, savedByUser, likedByUser, repostedByUser])
        ...
        <div className="mt-5 flex items-center justify-center">
          <div className="flex items-center gap-1 rounded-full p-1 border border-transparent">
            ...
            <button
              ...
              className={`group flex items-center gap-1.5 px-3 py-2 rounded-full transition-all duration-200 border ${
                isLiked
                  ? 'text-red-500 bg-red-50 border-red-200 shadow-sm'
                  : 'text-gray-500 hover:bg-red-50/60 hover:text-red-500 hover:border-red-100 border-transparent'
              } ${activeButton === 'like' ? 'scale-95' : 'hover:scale-105'}`}
            ...
            <button
              ...
              className={`group flex items-center gap-1.5 px-3 py-2 rounded-full transition-all duration-200 border ${
                isSaved
                  ...
                  : 'text-gray-500 hover:bg-amber-50/60 hover:text-amber-600 hover:border-amber-100 border-transparent'
              } ${activeButton === 'save' ? 'scale-95' : 'hover:scale-105'}`}
              title={!getAccessToken() ? 'Sign in to save this chronology' : undefined}
            >
              <Bookmark
                className={`w-5 h-5 transition-all duration-200 ${
                  isSaved ? 'fill-current text-amber-600' : 'group-hover:scale-110'
                }`}
              />
            </button>
            ...
            <button
              ...
              className={`group flex items-center gap-1.5 px-3 py-2 rounded-full transition-all duration-200 border ${
                isReposted
                  ...
                  : 'text-gray-500 hover:bg-green-50/60 hover:text-green-600 hover:border-green-100 border-transparent'
              } ${activeButton === 'repost' ? 'scale-95' : 'hover:scale-105'}`}
            ...
            <button
              ...
              className={`group flex items-center gap-1.5 px-3 py-2 rounded-full transition-all duration-200 border ${
                activeButton === 'share'
                  ? 'text-blue-600 bg-blue-50 border-blue-200 scale-95'
                  : 'text-gray-500 hover:bg-blue-50/60 hover:text-blue-600 hover:border-blue-100 border-transparent hover:scale-105'
              }`}
