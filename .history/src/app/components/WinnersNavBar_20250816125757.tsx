...
export default function WinnersNavbar() {
  ...
  return (
    ...
    >
      <nav className="max-w-5xl mx-auto px-4 py-4 flex items-center ">
        {/* Left - Logo and Title */}
        ...
        <div className="flex items-center space-x-4">
          <Link
            href="/wins"
            className="group relative inline-flex items-center px-4 lg:px-6 py-2.5 text-sm font-semibold rounded-full text-black dark:text-white hover:scale-105 active:scale-95 transition-all duration-300 z-10"
          >
            {/* Glowing, Animated Ring */}
            <span className="before:animate-spin-slow absolute -inset-[2px] rounded-full z-[-1] before:content-[''] before:absolute before:inset-0 before:rounded-full before:bg-[conic-gradient(from_0deg,_#ff6ec4,_#ffc3a0,_#ff6ec4)] before:blur-md before:opacity-70" />
            {/* Transparent Base */}
            <span className="absolute inset-0 rounded-full bg-white dark:bg-black z-[-2]" />
            <PlusCircle className="w-4 h-4 mr-2 group-hover:rotate-90 transition-transform duration-300" />
            DROP IT
          </Link>

          <Link href="/profile" aria-label="Go to your account" className="p-2  transition">
            <Image src={pippin} alt="Media Placeholder" width={100} height={100} />
          </Link>
