...
import { authFetch } from '../lib/api' // <-- your authFetch wrapper
...
export default function ChronologyCard(props: ChronologyCardHydrated) {
  ...
  // All API interactions now go through authFetch
  const interact = async (type: 'like' | 'repost' | 'share') => {
    try {
      await authFetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/gurkha/chronology/${id}/${type}`, {
        method: 'GET',
      })
