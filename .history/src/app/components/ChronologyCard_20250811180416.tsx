  ...
  const interact = async (type: 'like' | 'save' | 'repost' | 'share') => {
    ...
    try {
      ...
      if (type === 'like') {
        ...
        showNotification(data.liked ? 'F with it!' : 'Removed like', 'success')
      } else if (type === 'save') {
        setIsSaved(data.saved)
        showNotification(data.saved ? '📚 Saved!' : 'Removed from saved', 'success')
      } else if (type === 'repost') {
        setIsReposted(data.reposted)
        showNotification(data.reposted ? '🔄 Reposted!' : 'Removed repost', 'success')
      } else if (type === 'share') {
        // Try native sharing if available, fallback to copying URL
        try {
          if (navigator.share) {
