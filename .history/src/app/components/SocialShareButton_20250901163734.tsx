'use client'

import React from 'react'
import InstagramShareButton from './InstagramShareButton'

interface SocialShareButtonsProps {
  win: any
}

export default function SocialShareButtons({ win }: SocialShareButtonsProps) {
  const shareUrl = typeof window !== 'undefined' ? window.location.href : ''

  const shareOnX = () => {
    const tweetText = `Redub @${win.username} ${win.title}\n\n${shareUrl}`
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}`
    window.open(twitterUrl, '_blank')
  }

  return (
    <div className="flex gap-3 justify-center mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex-wrap">
      <button
        onClick={shareOnX}
        className="flex items-center gap-2 px-6 py-3 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors text-sm font-medium"
      >
        {/* X/Twitter Icon */}
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
        Redub on X
      </button>

      <InstagramShareButton win={win} />
    </div>
  )
}
