...
// Parse URL parameters to extract card ID and slug
export function parseCardUrl(pathname: string): { cardId: string | null; slug: string | null } {
  const segments = pathname.split('/').filter(Boolean)

  if (segments[0] !== 'company' || segments.length < 2) {
    return { cardId: null, slug: null }
  }
...
// Validate if a slug matches the expected format for a card
export function validateSlug(providedSlug: string, expectedSlug: string): boolean {
  // Exact match
  if (providedSlug === expectedSlug) return true

  // Allow some flexibility for URL encoding differences
  ...
  return normalizeSlug(providedSlug) === normalizeSlug(expectedSlug)
}
...
export function getRedirectUrl(
  ...
  const { cardId, slug: providedSlug } = parseCardUrl(currentPath)

  if (!cardId || cardId !== card.cardId) {
    return null // This shouldn't happen if the card was loaded successfully
  }

  const correctSlug = createCardSlug(card.companyName, card.cardTicker)
  const correctUrl = `${baseUrl}/company/${card.cardId}/${correctSlug}`

  // If no slug provided, redirect to SEO-friendly URL
  if (!providedSlug) {
    return correctUrl
  }

  // If slug doesn't match, redirect to correct slug
  if (!validateSlug(providedSlug, correctSlug)) {
    return correctUrl
  }
