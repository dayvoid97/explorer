    ...
    <div className="min-h-screen px-4 py-10 max-w-5xl mx-auto space-y-12">
      <section className="border rounded-xl shadow p-6 flex flex-col sm:flex-row items-center gap-6">
        <div className="w-24 h-24 rounded-full overflow-hidden border-2">
          {profile.profilePicUrl ? (
            <img src={profile.profilePicUrl} alt="Profile Picture" className="object-cover w-full h-full" />
          ) : (
          ...
          <div className="flex justify-between items-center flex-wrap gap-2">
            ...
            <div className="flex gap-2">
              <Link href={`/message?to=${profile.username}`}>
                <button aria-label="Message user" className="hover:bg-gray-100 dark:hover:bg-gray-800 px-3 py-2 rounded-md">
                  <Mail size={20} />
                </button>
              </Link>
              ...
              {profile.socialLinks.map((link) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => handleSocialLinkClick(link.social_identifier, link.social_id, link.social_link)}
        ...
        {profile.wins?.length ? (
          <>
            <div className="grid sm:grid-cols-2 gap-4">
              {(showAllWins ? profile.wins : profile.wins.slice(0, 6)).map((win, idx) => (
                <div
                  key={idx}
      ...
      <section>
        ...
        {profile.cards?.length ? (
          <>
            <div className="grid sm:grid-cols-2 gap-4">
              {(showAllCards ? profile.cards : profile.cards.slice(0, 6)).map((card, idx) => (
                <div
                  key={idx}
