  ...
  return (
    <main className="max-w-6xl mx-auto py-16 px-6">
      ...
      <form onSubmit={handleSubmit} className="flex  flex-col gap-6 mb-12 mt-10">
        <div className="items-center sm:flex-row gap-4">
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="🔍 ENTER THE TITLE"
            className="py-100 w-full sm:w-1/2 px-4 border  rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm"
          />
        </div>
        <div className=" flex-col items-center  sm:flex-row gap-4">
          <input
            type="text"
            value={countriesInput}
            onChange={(e) => setCountriesInput(e.target.value)}
            placeholder="CATEGORIES"
            className="w-full sm:w-1/4 px-4 py-3 border  rounded-xl focus:outline-none focus:ring-2 gap-10 focus:ring-green-500 shadow-sm"
          />
          <input
            type="text"
            value={usernameInput}
