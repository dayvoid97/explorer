...
export default function MessagePage() {
  ...
  // Step 1: Initialize chat and fetch message history
  useEffect(() => {
    ...
    const initChat = async () => {
      try {
        ...
        if (!initRes.ok) throw new Error(chatData.message || 'Failed to initiate chat')

        setChat(chatData)

        // Fetch messages
        const msgRes = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/messages/fetch/${chatData.connectionId}`,
          {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (!msgRes.ok) throw new Error(msgData.message || 'Failed to fetch messages')
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/messages/post`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
