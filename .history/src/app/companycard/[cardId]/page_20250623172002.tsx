...
export default function CompanyCardPage() {
  ...
  useEffect(() => {
    const fetchCard = async () => {
      ...
      try {
        ...
        const res = await authFetch(`${API_BASE}/gurkha/companycard/view?cardId=${cardId}&light=true`, {
          headers: { /* authFetch adds Authorization header */ },
        })
        const data = await res.json()
        if (!res.ok) {
          throw new Error(data.message || data.error || `Failed to fetch card metadata (Status: ${res.status})`)
        }
        setCard(data.card)
      } catch (err: any) {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        ...
        const data = await res.json()
        if (!res.ok) {
          throw new Error(data.message || data.error || `Failed to fetch items (Status: ${res.status})`)
        }
  ...
  const handleUpdateMetadata = async () => {
    ...
    if (!changesMade) { // No actual changes, just exit edit mode
    ...
    try {
      ...
      setCard({ ...card, companyName: payload.title || card.companyName, cardTicker: payload.ticker || card.cardTicker })
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      // CHANGED: Use authFetch for this authenticated call
      const res = await authFetch(
        `${API_BASE}/gurkha/companycard/item/${confirmDeleteItemId}`,
        {
          method: 'DELETE',
          headers: { /* authFetch adds Authorization header */ },
        }
  ...
  if (loading) return <p className="p-4 text-center text-gray-500 dark:text-gray-400">Loading card...</p>
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-6 bg-white dark:bg-gray-900 min-h-screen rounded-lg shadow"> {/* Dark mode bg/shadow */}
      <div className="flex items-center gap-3 mb-6">
        <div className="flex flex-col gap-2 flex-1"> {/* flex-1 to push button */}
          <input
            type="text"
            value={editTitle}
      ...
      <div className="flex items-center gap-4 mb-2">
        <span
          className={`text-xs font-semibold px-2 py-1 rounded ${
            card.isPublished ? 'bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-950 dark:text-yellow-300'
          }`}
        >
        ...
        <button
          ...
          className={`text-sm font-medium ${
            card.isPublished ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'
          } hover:underline`}
        >
        ...
        <div className="flex-1 space-y-3">
          {loadingItems ? (
            ...
            items.map((item: any, i: number) => (
              <div key={i} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm px-4 py-3 border dark:border-gray-700">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1 dark:text-gray-300">{item.description}</p>
      ...
      {confirmDeleteItemId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm text-center">
            <p className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Confirm Deletion</p>
            <p className="text-gray-700 dark:text-gray-300 mb-6">Are you sure you want to delete this item? This action cannot be undone.</p>
