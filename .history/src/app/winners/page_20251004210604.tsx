...
export default function WinnersPage() {
  ...
  const adSlots = [
    '5884450985', // Replace with your actual ad slot IDs
  ]

  return (
    <div className="bg-[#3b3a3c] flex-grow" style={{ scrollBehavior: 'smooth' }}>
      <Navbar />
      ...
      {loading && wins.length === 0 ? (
        <div className="bg-[#3b3a3c] text-center text-gray-500 dark:text-gray-300 py-20">
          <div className="animate-pulse">Loading wins...</div>
        </div>
      ) : wins.length === 0 && !hasMore ? (
        <div className="text-center bg-[#3b3a3c] dark:text-gray-300 py-20">
          No wins found for this category.
        </div>
      ) : (
        <div className="flex-grow mt-10 grid sm:grid-cols-2 md:grid-cols-3 gap-5 ml-20 mr-20 items-start">
          {wins.map((win, index) => {
            const elements: React.ReactNode[] = []

            // Always render the win card
            elements.push(<WinCard win={win} key={win.id} />)

            // If this index is in randomAdIndices, insert an ad **after** the win
            if (randomAdIndices.has(index)) {
              const adId = adSlots[index % adSlots.length] // rotate adSlots
              elements.push(
                <div key={`ad-${index}`} className="w-full">
                  {/* Replace this div with your actual AdUnit */}
                  <div className="bg-gray-700 text-white text-center p-4 rounded-md">
                    Ad Slot: {adId}
                  </div>
                </div>
              )
