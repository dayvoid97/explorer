...
import CommentSection from '@/app/components/CommentSection' // <-- Uncommented and correctly imported
      ...
      {win.mediaUrls?.length > 0 && (
        <section className="grid sm:grid-cols-2 gap-5">
          {win.mediaUrls.map((url: string, idx: number) => (
            <div
              key={idx}
              className="group bg-muted/20 overflow-hidden rounded-xl border transition hover:shadow-lg"
            >
              {/* Basic check for video vs image, assuming common extensions */}
