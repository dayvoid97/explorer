    ...
    try {
      ...
      if (isUsingChronology) {
        ...
        if (forceReload || isCacheStale) {
          const freshData = await fetchExploreWins({ sortBy, limit: 20 })
          navData = freshData
          setExploreCache((prev) => ({
            ...prev,
            [sortBy]: { data: freshData, timestamp: Date.now() },
          }))
        } else {
          navData = cacheEntry.data
        }
      }
      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b dark:border-gray-800 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex flex-col items-center text-center space-y-1">
          ...
          <ShuffleToggle
            ...
            show={!isUsingChronology} // Show only when not in a chronology
          />
