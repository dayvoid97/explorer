    ...
    <div className="relative">
      ...
      >
        ...
        <button
          ...
          className="left-200 absolute top-1/2 transform -translate-y-1/2 w-10 h-10 bg-black/50 text-white rounded-full flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-200 hidden sm:flex"
...
function SortToggle({
  ...
  sortMode: 'recent' | 'hottest' | 'shuffle'
  onToggle: () => void
  show: boolean
}) {
  if (!show) return null
  const buttonText = sortMode === 'hottest' ? 'Hot' : sortMode === 'shuffle' ? 'Shuffle' : 'Recent'
  const buttonClasses = `
  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={onToggle} className={buttonClasses} title={`Change to ${buttonText}`}>
        <Shuffle
          size={14}
          className={`transition-transform ${sortMode !== 'recent' ? 'text-orange-500' : ''}`}
        />
        {buttonText}
      </button>
    </div>
