...
export default function WinDetailPage({ winId }: Props) {
  ...
  const loadNavigationData = async (winData: any, forceReload = false) => {
    ...
    try {
      ...
      // Step 1: Always fetch chronology data if available. This list takes precedence.
      if (isUsingChronology) {
        const chronologyId = winData.chronologies[0].id
        chronologyData = await fetchChronology(chronologyId)
      }

      // Step 2: Fetch or use cached explore data based on shuffleMode.
      // We will always fetch this regardless of chronology, as it's the fallback.
      ...
      // Step 3: Combine the lists, prioritizing chronology wins.
      if (isUsingChronology) {
        ...
        // If not in a chronology, navigation is purely based on explore wins.
        navData = exploreData
      }

      // Step 4: Update state with the final navigation data.
  ...
  const showShuffleToggle = navigationData.length > 1 && !isUsingChronology
      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b dark:border-gray-800 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex flex-col items-center text-center space-y-1">
          ...
          <ShuffleToggle
            ...
            show={true} // Always show the toggle button
          />
