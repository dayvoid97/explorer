...
export default function WinDetailPage({ winId }: Props) {
  ...
  const loadNavigationData = async (winData: any, forceReload = false) => {
    ...
    try {
      ...
      if (winData.chronologies && winData.chronologies.length > 0) {
        // Load chronology data (always fresh since it's the main content)
        const chronologyId = winData.chronologies[0].id
        chronologyData = await fetchChronology(chronologyId)
        console.log('📚 Loaded chronology data:', chronologyId, chronologyData.length, 'items')
      }
      ...
      if (forceReload || !exploreCache[sortBy] || exploreCache[sortBy].length === 0) {
        ...
        exploreData = await fetchExploreWins({
          sortBy,
          limit: 20, // Keep reasonable limit
        })
      ...
      if (chronologyData.length > 0) {
        // Combine chronology + explore wins for seamless navigation
        // Remove duplicates from explore data if they exist in chronology
        ...
        console.log(
          '🔗 Combined navigation:',
          chronologyData.length,
          'chronology +',
          uniqueExploreData.length,
          'explore =',
          navData.length,
          'total'
        )
      } else {
        // Use only explore wins
        navData = exploreData
        console.log('🎯 Using explore wins navigation only:', navData.length, 'items')
      }
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/gurkha/wins/${winId}`)
      ...
      .then((data) => {
        ...
        // Load navigation data (initial load)
  ...
  // Reload navigation when shuffle mode changes (always reload now since we always use explore data)
  useEffect(() => {
    if (win) {
