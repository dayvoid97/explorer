...
function MediaCarousel({
  ...
  // Note: The original code shuffles the array here but the result is not used.
  // The media items should not be shuffled in the carousel. I've left the original logic as is.
...
export default function WinDetailPage({ winId }: Props) {
  ...
  const loadNavigationData = async (winData: any, forceReload = false) => {
    ...
    try {
      ...
      const isUsingChronology = winData.chronologies && winData.chronologies.length > 0
      ...
      const cacheEntry = exploreCache[sortBy]
      ...
      if (forceReload || isCacheStale) {
        ...
        setExploreCache((prev) => ({
          ...prev,
          [sortBy]: { data: exploreData, timestamp: Date.now() },
        }))
      } else {
  ...
  // New function to load more explore wins
  const loadMoreExploreWins = async () => {
    if (isLoadingNavigation) return // Prevent multiple calls
    setIsLoadingNavigation(true)
    try {
      const sortBy = shuffleMode ? 'hottest' : 'recent'
      const offset = navigationData.length // Start from the end of the current list

      const newExploreData = await fetchExploreWins({
        sortBy,
        limit: 20, // Fetch the next 20 items
      })

      // Append new data to the existing navigation data list
      setNavigationData((prevData) => [...prevData, ...newExploreData])
    } catch (error) {
      console.error('❌ Error loading more explore wins:', error)
    } finally {
      setIsLoadingNavigation(false)
    }
  }
  ...
  const goToNext = () => {
    ...
    // Check if we are at the end of the list and need to load more
    if (currentIndex >= navigationData.length - 5) {
      // We are within 5 items of the end, trigger a load of more wins
      loadMoreExploreWins()
    }
  }
  ...
  const hasNext = currentIndex < navigationData.length - 1 || isLoadingNavigation
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b dark:border-gray-800 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex flex-col items-center text-center space-y-1">
          ...
          <ShuffleToggle shuffleMode={shuffleMode} onToggle={handleShuffleToggle} show={true} />
