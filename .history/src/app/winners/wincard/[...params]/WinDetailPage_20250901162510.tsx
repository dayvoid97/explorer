...
function SocialShareButtons({ win }: { win: any }) {
  ...
  const shareOnTwitter = () => {
    // Format: ReDub + newline + Title + newline + URL
    const tweetText = `ReDub\n${title}\n${shareUrl}`
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}`
    window.open(twitterUrl, '_blank')
  }

  return (
    <div className="flex gap-2 justify-center mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
      <button
