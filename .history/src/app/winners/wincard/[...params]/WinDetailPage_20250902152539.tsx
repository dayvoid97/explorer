...
import { fetchExploreWins } from '@/app/lib/fetchWins' // `fetchChronology` is removed from this import
...
export default function WinDetailPage({ winId }: Props) {
  ...
  // New: Fisher-Yates (aka Knuth) Shuffle algorithm
  ...
  const loadNavigationData = async (winData: any, forceReload = false) => {
    ...
    try {
      ...
      // Use the cached data if it's not stale and we're not forcing a reload
      ...
      if (forceReload || isCacheStale) {
        ...
        // Store the original fetched data in cache.
      ...
      // Conditionally shuffle the data for navigation
      const navData = shuffleMode ? shuffleArray([...exploreData]) : exploreData
  ...
  // Simplify this since we are ignoring chronology now
  ...
  // Re-evaluate what's shown to the user since we're ignoring chronology.
  // The 'isUsingChronology' variable is now obsolete in the context and can be removed.
  const showShuffleToggle = navigationData.length > 1
