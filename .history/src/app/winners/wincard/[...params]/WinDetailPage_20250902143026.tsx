...
function MediaCarousel({
  ...
  return (
    <div className="relative">
      ...
      {/* Dots Indicator */}
      <div className="background-black flex justify-center mt-4 space-x-2">
...
export default function WinDetailPage({ winId }: Props) {
  ...
  const loadNavigationData = async (winData: any) => {
    ...
    try {
      ...
      if (winData.chronologies && winData.chronologies.length > 0) {
        ...
        // Fall back to explore wins with shuffle/recent mode
        const sortBy = shuffleMode ? 'hottest' : 'recent'
        navData = await fetchExploreWins({
          sortBy,
          limit: 100, // Adjust limit as needed
        })
        console.log(`🎯 Using explore wins navigation (${sortBy}):`, navData.length, 'items')
      }
  ...
  // Reload navigation when shuffle mode changes (only for non-chronology navigation)
  useEffect(() => {
    if (win) {
      // Only reload if not using chronology navigation
      if (!win.chronologies || win.chronologies.length === 0) {
  ...
  const showShuffleToggle = !isUsingChronology && navigationData.length > 1
            ...
            <div className="flex items-center justify-center gap-4 mt-2">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 px-2">
                ...
                {win.chronologies?.length > 0 && (
                  <div className="text-xs text-blue-400">📚 {win.chronologies[0].name}</div>
                )}
              </div>
