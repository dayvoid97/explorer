...
import { fetchExploreWins, fetchChronology } from '@/app/lib/fetchWins'
...
function ShuffleToggle({
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        onClick={onToggle}
        className={`flex items-center gap-2 px-3 py-1 rounded-full border text-xs transition-colors ${
          shuffleMode
        ...
        title={shuffleMode ? 'Showing hottest wins' : 'Showing recent wins'}
      >
        <Shuffle
          size={14}
          className={`transition-transform ${shuffleMode ? 'text-orange-500' : ''}`}
        />
        {shuffleMode ? 'Hot' : 'Recent'}
      </button>
      <span className="text-xs text-gray-400">for variety</span>
    </div>
  )
}
...
export default function WinDetailPage({ winId }: Props) {
  ...
  const loadNavigationData = async (winData: any, forceReload = false) => {
    console.log('🔍 Input winData:', winData)
    console.log('🔍 winData type:', typeof winData)
    console.log('🔍 winData.chronologies:', winData.chronologies)

    setIsLoadingNavigation(true)
    try {
      let navData: any[] = []
      let chronologyData: any[] = []
      let exploreData: any[] = []

      if (winData.chronologies && winData.chronologies.length > 0) {
        const chronologyId = winData.chronologies[0].id
        console.log('🔍 Chronology ID:', chronologyId)
        chronologyData = await fetchChronology(chronologyId)
        console.log('🔍 Chronology data:', chronologyData)
        console.log('🔍 Chronology data length:', chronologyData.length)
        console.log('🔍 First chronology item:', chronologyData[0])
      }

      // Load or use cached explore data
      const sortBy = shuffleMode ? 'hottest' : 'recent'
      console.log('🔍 Sort by mode:', sortBy)

      if (forceReload || !exploreCache[sortBy] || exploreCache[sortBy].length === 0) {
        // Fetch fresh explore data
        console.log(`🔄 Fetching fresh explore wins (${sortBy})...`)
        exploreData = await fetchExploreWins({
          sortBy,
          limit: 20,
        })
        console.log('🔍 Fresh explore data:', exploreData)
        console.log('🔍 Fresh explore data sample:', exploreData.slice(0, 3))
        ...
        console.log(`✅ Cached ${exploreData.length} ${sortBy} wins`)
      } else {
        exploreData = exploreCache[sortBy]
        console.log(`📋 Using cached explore wins (${sortBy}):`, exploreData.length, 'items')
        console.log('🔍 Cached explore data sample:', exploreData.slice(0, 3))
      }

      if (chronologyData.length > 0) {
        const chronologyIds = new Set(chronologyData.map((item: any) => item.id))
        console.log('🔍 Chronology IDs set:', chronologyIds)

        const uniqueExploreData = exploreData.filter((item: any) => !chronologyIds.has(item.id))
        console.log('🔍 Unique explore data after filtering:', uniqueExploreData.length, 'items')

        navData = [...chronologyData, ...uniqueExploreData]
      } else {
        navData = exploreData
      }

      console.log('🔍 Final navData:', navData)
      console.log('🔍 Final navData length:', navData.length)
      console.log('🔍 Final navData sample:', navData.slice(0, 3))
  ...
  const isUsingChronology = win?.chronologies?.length > 0
  const showShuffleToggle = navigationData.length > 1 // Always show if we have multiple items

  // Helper to determine if we're in chronology section vs explore section
  const getNavigationContext = () => {
    if (!isUsingChronology)
      return { section: 'explore', position: currentIndex + 1, total: navigationData.length }

    // Find chronology length by checking chronologies
    const chronologyLength = win.chronologies[0]?.winsCount || 0
    if (currentIndex < chronologyLength) {
      return { section: 'chronology', position: currentIndex + 1, total: chronologyLength }
    } else {
      return {
        section: 'explore',
        position: currentIndex - chronologyLength + 1,
        total: navigationData.length - chronologyLength,
      }
    }
  }
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b dark:border-gray-800 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex flex-col items-center text-center space-y-1">
            ...
            <div className="flex items-center justify-center gap-4 mt-2">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 px-2">
                {(() => {
                  ...
                  return (
                    <>
                      {currentIndex + 1} of {navigationData.length}
                      {isUsingChronology && (
                        <div className="text-xs text-blue-400">
                          {context.section === 'chronology'
                            ? `📚 ${win.chronologies[0].name} (${context.position}/${context.total})`
                            : `🎯 Exploring (${context.position}/${context.total})`}
                        </div>
                      )}
                    </>
                  )
          ...
          {/* Shuffle Toggle - Only show when not in chronology mode */}
          <ShuffleToggle
            shuffleMode={shuffleMode}
            onToggle={handleShuffleToggle}
            show={showShuffleToggle}
          />
