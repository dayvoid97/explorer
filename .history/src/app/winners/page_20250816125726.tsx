// ✅ Fixed WinnersPage with Sorting and Load More - Removed stale closure issue
'use client'
  ...
  const loadWins = useCallback(
    async (isInitialLoad: boolean) => {
      ...
      try {
        ...
        if (isInitialLoad) {
          // For initial load, just set the fetched data directly
          setWins(fetchedData)
        } else {
          // For load more, use functional state update to avoid stale closure
          setWins((prevWins) => {
            // Filter out duplicates using the current state directly
            const newUniqueWins = fetchedData.filter(
              (newWin) => !prevWins.some((existingWin) => existingWin.id === newWin.id)
            )

            // Combine previous wins with new unique wins
            const combinedWins = [...prevWins, ...newUniqueWins]

            // Additional deduplication using Map to ensure uniqueness
