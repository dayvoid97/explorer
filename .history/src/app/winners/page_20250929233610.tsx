  ...
  return (
    <div style={{ scrollBehavior: 'smooth' }}>
      ...
      {loading && wins.length === 0 ? (
        ...
        <div className="bg-[#3b3a3c] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4">
          {wins.map((win, index) => (
            <React.Fragment key={win.id}>
              {/* Win Card */}
              <div className="transition-all duration-300 ease-in-out transform hover:scale-[1.02] p-20">
                <WinCard win={win} />
              </div>
            </React.Fragment>
          ))}
