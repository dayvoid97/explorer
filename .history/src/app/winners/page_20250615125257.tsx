...
export default function WinnersPage() {
  ...
  // Use a ref to hold the current `wins` state for deduplication within useCallback
  // This ref is updated *after* `setWins`, ensuring `loadWins` closure doesn't become stale.
  ...
  const loadWins = useCallback(
    async (isInitialLoad: boolean) => {
      ...
      try {
        ...
        // Filter out any duplicates using the *current* state of wins via winsRef
        ...
        if (isInitialLoad) {
          ...
          setWins((prevWins) => {
            // This setWins uses the prevWins directly from the functional update,
            // which is the most up-to-date state, combined with newUniqueWins
    ...
    [sortBy] // <--- THIS IS THE KEY FIX: Removed `wins` from dependencies.
  )
  ...
  }, [wins]) // `adIndices` legitimately depends on `wins` to recalculate ad positions
