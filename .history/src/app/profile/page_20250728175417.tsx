...
import SocialLinksProfile from '../components/ui/SocialLinks' // Correct import path
...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      setBio(data.bio || '')
      setUserChronologies(data.chronologies || [])
    } catch (err: any) {
  ...
  const fetchFollowersOrFollowing = async (type: 'followers' | 'following') => {
    setLoadingList(true)
    try {
      const res = await authFetch(`${API_BASE}/gurkha/profile/${type}`)
      const data = await res.json()
      if (type === 'followers') setFollowers(data.users || [])
  ...
  //   try {
  //     const res = await authFetch(`${API_BASE}/gurkha/chronologies`)
  //     const data = await res.json()
  //     if (res.ok && Array.isArray(data.chronologies)) {
  //       setUserChronologies(data.chronologies)
  //     } else {
  //       setUserChronologies([])
  //     }
      ...
      <section className="border border-gray-200 rounded-xl shadow-sm p-6">
        ...
        <div className="flex gap-8 justify-center my-4">
          ...
          <button className="text-center hover:underline">
            <div className="text-lg font-bold">{profile.chrono ?? 0}</div>
            <div className="text-xs text-gray-500">Following</div>
          </button>
        </div>
