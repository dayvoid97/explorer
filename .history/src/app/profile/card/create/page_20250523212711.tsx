...
export default function CreateCardPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Track and redirect to the dynamic card route
      if (data?.card?.cardId) {
        trackEvent('card_created_or_found', {
          ticker: data.card.cardTicker,
          status: data.status,
        })
        router.push(`/companycard/${data.card}`)
      } else {
        throw new Error('Card ID missing in server response')
      }
    } catch (err: any) {
