...
import { removeTokens } from '../lib/auth' // Now refers to plural removeTokens
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetchProfile()
  }, [router]) // Add router to dependencies if you use it in fetchProfile (for handleAuthRedirect)
    ...
    <main className="max-w-5xl mx-auto py-10 px-4 sm:px-6 space-y-12">
      ...
      <div className="flex justify-center gap-4 mt-10">
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium ${!showSavedWins ? '' : ''}`}
        >
          My Wins
        </button>
        <button
          onClick={() => setShowSavedWins(true)}
          className={`px-4 py-2 rounded-md text-sm font-medium ${showSavedWins ? '' : 'bg--100'}`}
        >
          Saved Wins
        </button>
      </div>
