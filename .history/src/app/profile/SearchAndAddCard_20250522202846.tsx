...
export default function SearchAndAddCard() {
  ...
  const handleLaunch = async (item: ExplorerItem) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/gurkha/card/launch`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok && data.status === 'exists') {
        ...
        window.location.href = `/profile/card/create?ticker=${item.ticker}&ref=${encodeURIComponent(
          data.userRef
        )}`
      } else {
