    ...
    <section className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {cards.map((card) => (
          <div
            key={card.cardId}
            className="bg-white border hover:shadow-md transition rounded-xl p-5 flex flex-col justify-between"
          >
            <div className="space-y-1">
              <h3 className="text-base font-semibold text-gray-800 line-clamp-2">
                {card.companyName}
              </h3>
              <p className="text-sm text-gray-500">Ticker: {card.cardTicker}</p>
              <span
