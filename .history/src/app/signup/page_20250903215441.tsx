...
export default function SignUpPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Signup successful! A verification email has been sent. You can log in now.')
      setTimeout(() => {
        router.push('/login')
      ...
      {step === 'signup' ? (
        <form onSubmit={handleSignup} className="space-y-4">
          <input
            ...
            required
            className="color-whote w-full px-4 py-2 border border-gray-300 rounded-md"
          />
