...
export default function CompanyCardPublicPage() {
  ...
  const providedSlug = Array.isArray(params) && params.length > 1 ? params.slice(1).join('/') : null

  const { data: card, loading } = usePublicCard(cardId as string)

  // Handle SEO and URL normalization
  useEffect(() => {
    if (card && !loading) {
      updateCardMetadata(card)
      updateCanonicalUrl(card)

      // Check if we need to redirect to correct URL
      const redirectUrl = getRedirectUrl(window.location.pathname, card)

      if (redirectUrl) {
        console.log('Redirecting to SEO-friendly URL:', redirectUrl)
