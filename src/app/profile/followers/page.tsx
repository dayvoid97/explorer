...
export default function FollowersPage() {
  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      ...
      setError("");
      try {
        const res = await authFetch(`${API_BASE}/gurkha/profile/followers`);
  ...
  return (
    <main className="max-w-2xl mx-auto py-10 px-4">
      <h1 className="text-2xl font-bold mb-6 text-center">Followers</h1>
      {loading && <p className="text-center">Loading...</p>}
      {error && <p className="text-center text-red-600">{error}</p>}
      {!loading && !error && (
        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
          {followers.length === 0 ? (
            <li className="py-4 text-center text-gray-500">No followers yet.</li>
          ) : (
            followers.map((user) => (
              <li
                key={user.username}
